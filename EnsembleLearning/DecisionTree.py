...
def chooseBestAttribute(S, Attributes, target, EntropyGiniMajority, weights,
                        featureSize):
    '''
        ...
        EntropyGiniMajority == 3, uses Entropy with weights
        EntropyGiniMajority == 4, sample features for Random Forest
    '''
    dictionary = {}
    if EntropyGiniMajority == 4:
        # Get sample subset of remaining features
        if len(Attributes) - 1 < featureSize:
            randomSample = random.sample(
                range(len(Attributes) - 1),
                len(Attributes) - 1)
        else:
            # Get sample from total features
            randomSample = random.sample(
                range(len(Attributes) - 1), featureSize)
        # If more than one sample, add attributes
        if len(randomSample) > 1:
            newAttributes = [Attributes[x] for x in randomSample]
        # else add single attribute remaining
        else:
            newAttributes = [Attributes[randomSample[0]]]
        # For the subset, get entropy gain
            ...
            if attr != target:
                ...
                if EntropyGiniMajority == 0:
                    ...
                    dictionary[attr] = getInformationGainGiniIndex(
                        S, Attributes, target, attr)
                elif EntropyGiniMajority == 2:
                    dictionary[attr] = getInformationGainMajorityError(
                        S, Attributes, target, attr)
                else:
                    dictionary[attr] = getInformationGainWithWeightst(
                        S, Attributes, target, attr, weights)
...
def getInformationGainWithWeightst(S, Attributes, target, attr, weights):
    '''
        Helper Function that Calculates Gain using Entropy with weights
    '''
    # Entropy of whole dataset
    mainEntropy = calculateEntropyWeights(S, Attributes, target, weights)
    indx = getIndexOfAttributes(Attributes, attr)
    # Add frequency of attributes values to dictionary
    dictionary = helperFrequencyFinder(S, indx)[0]
    attrEntropy = 0.0
    ...
    for num in dictionary.keys():
        subset = [col for col in S if col[indx] == num]
        attrEntropy += weights[dictionary[num] - 1] * (
            dictionary[num] / total) * calculateEntropyWeights(
                subset, Attributes, target, weights)
    #Return gain
    return mainEntropy - attrEntropy
...
def calculateEntropyWeights(S, Attributes, target, weights):
    '''
    ...
    for num in values:
        entropy += (-weights[num]) * (math.log2(weights[num]))
    return entropy
...
def ID3(S, Attributes, Label, EntropyGiniMajority, treeDepth, weights,
        featureSize):
    '''
    ...
    return recursiveID3(S, Attributes, Label, EntropyGiniMajority,
                        currentDepth, treeDepth, weights, featureSize)


def recursiveID3(S, Attributes, Label, EntropyGiniMajority, currentDepth,
                 treeDepth, weights, featureSize):
    '''
    ...
    if not S or len(Attributes) == 1:
        ...
        for v in valuesOfAList:
            ...
            # If next depth will be max depth, choose most common from subset
                ...
                childOfChild = recursiveID3(
                    subset, removedBestAttribute, Label, EntropyGiniMajority,
                    currentDepth + 1, treeDepth, weights, featureSize)
